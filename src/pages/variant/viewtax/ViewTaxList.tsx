...
import { getTaxes, deleteTax } from "../models/tax.api";
import type { Tax } from "../models/tax.api";
...
const ViewTaxList = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            console.error(err.message || "Failed to load taxes");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="view-colour-container">
            <div className="view-colour-header">
                <h2>View Tax List</h2>
                <button className="btn-add-new-colour" onClick={() => navigate("/variant/add-tax")}>
                    Add New Tax
                </button>
            </div>
            <div className="view-colour-card">
                ...
                <div className="table-wrapper">
                    <table className="redesign-table">
                        <thead><tr><th># ↕</th><th>Tax Name ↕</th><th>Value (%) ↕</th><th>Status ↕</th><th>Action ↕</th></tr></thead>
                        <tbody>
                            {currentData.length === 0 ? (<tr><td colSpan={5} style={{ textAlign: "center" }}>No data</td></tr>) : (
                                    ...
                                    <tr key={t.id}>
                                        ...
                                        <td>
                                            <div className="action-icons">
                                                <button className="btn-icon-edit" onClick={() => navigate(`/variant/edit-tax/${t.id}`)}><SquarePen size={16} /></button>
                                                <button className="btn-icon-view" onClick={() => handleDelete(t.id)}><Eye size={16} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                ))
