...
const AddSubCheckboxMaster = () => {
    ...
    const [multiChecks, setMultiChecks] = useState<{ label: string; checked: boolean }[]>([{ label: "", checked: false }]);
    ...
    const handleAddInput = () => {
        setMultiChecks([...multiChecks, { label: "", checked: false }]);
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const checkedItems = multiChecks.filter(item => item.checked && item.label.trim() !== "");
        ...
        try {
            ...
            setMultiChecks([{ label: "", checked: false }]);
            loadData();
        } catch (err: any) {
            console.error(err);
    ...
    const handleEditClick = (cb: CheckboxMaster) => {
        setEditingId(cb.id);
        setEditForm({ name: cb.label, status: cb.status || (cb.is_active === 0 ? "inactive" : "active") });
        ...
        <div className="checkbox-page-container">
            ...
            <div className="checkbox-form-card">
                <div className="checkbox-form-header">
                    <h2>{editingId ? "Edit SubCheckbox Master" : "Add Checkbox Master"}</h2>
                </div>
                <hr className="checkbox-dashed-divider" />

                {/* <div style={{ marginBottom: '25px', width: '300px' }} className="checkbox-input-group">
                    <label>SubSector Title</label>
                    <select value={selectedSubsectorId} onChange={(e) => setSelectedSubsectorId(e.target.value)} disabled={editingId !== null}>
                        <option value="">Select SubSector</option>
                        {subsectors.map(s => <option key={s.id} value={s.id}>{s.sub_sector_name}</option>)}
                    </select>
                </div> */}
                ...
                {!editingId ? (
                    <form onSubmit={handleSave}>
                        ...
                        <div className="checkbox-action-footer">
                            ...
                            <button type="button" className="btn-check-premium btn-check-reset" onClick={() => setMultiChecks([{ label: "", checked: false }])}>
                                Reset
                            </button>
                            <button type="button" className="btn-check-premium btn-check-cancel" onClick={() => { setMultiChecks([{ label: "", checked: false }]); }}>
                                Cancel
                            </button>
                        </div>
                    </form>
