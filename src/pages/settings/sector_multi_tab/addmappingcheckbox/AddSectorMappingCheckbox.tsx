...
import { type SectorTitle } from "../../../../api/sectorTitle.api";
import {
    getMenus,
    getTabHeadings,
    getCheckboxes,
...
const AddSectorMappingCheckbox = () => {
    // Data Lists
    const [sectors] = useState<SectorTitle[]>([
        ...
        <div className="page-container" style={{ padding: "0 20px" }}>
            <div className="page-header" style={{ marginBottom: "25px" }}>
                <h1 style={{ fontSize: "24px", fontWeight: "700", color: "#1e293b" }}>
                    {isEdit ? "Edit Sector Mapping Checkbox" : "Add Sector Mapping Checkbox"}
                </h1>
            </div>
                    ...
                    <div style={{ display: "flex", gap: "25px", marginBottom: "40px" }}>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div className="custom-select-container" style={{ position: "relative" }}>
                                ...
                                >
                                    {(menuId && menus.find(m => m.id.toString() === menuId)?.name) || "select"}
                                    <span style={{ fontSize: "12px", color: "#64748b" }}>▼</span>
                                </div>
                                    ...
                                    <div className="custom-dropdown-menu" style={{
                                        ...
                                        <div style={{ padding: "4px" }}>
                                            {menus.length === 0 ? (
                                                ...
                                                menus.filter(m => (m.name || "").toLowerCase().includes(menuSearch.toLowerCase())).map(m => (
                                                    <div
                                                        key={m.id}
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                mappings.map((m, idx) => {
                                    ...
                                    const checkboxNames = currentCheckboxes.map((id: number) => checkboxes.find(c => c.id === id)?.checkbox_name).filter(Boolean).join(", ");

                                    return (
                                        <tr key={m.id}>
                                            <td>{idx + 1}</td>
                                            <td style={{ fontWeight: "600", color: "#64748b" }}>{currentSector?.title || (m as any).sector_title_name || (m as any).sectorTitle || (m as any).sector_name || "—"}</td>
                                            <td style={{ fontWeight: "600", color: "#334155" }}>{currentMenu?.name || (m as any).menu_name || (m as any).name || "—"}</td>
                                            <td>{currentHeading?.master_name || (m as any).heading_name || (m as any).master_name || "—"}</td>
