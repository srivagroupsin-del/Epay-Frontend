...
import { getMenus, type Menu as CategoryMultitabMenu } from "../../../../api/multitab/category_menu.api";
import { getTabHeadings, type Heading as CategoryTabHeading } from "../../../../api/multitab/category_heading.api";
import { getCheckboxes, type Checkbox as CategoryCheckboxMaster } from "../../../../api/multitab/category_checkbox.api";
import {
    getMappings,
    saveMapping as saveCategoryMapping,
    updateMapping as updateCategoryMapping,
    deleteMapping as deleteCategoryMapping
} from "../../../../api/multitab/category_mapping.api";

const AddCategoryMappingCheckbox = () => {
    // Data Lists
    ...
    const loadInitialData = async () => {
        try {
            setLoading(true);
            const [sData, mData, hData, cData, maps] = await Promise.all([
                getSectorTitles(),
                getMenus(),
                getTabHeadings(),
                getCheckboxes(),
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEdit && editId) {
                await updateCategoryMapping(editId, payload);
                alert("Mapping updated successfully ✅");
            } else {
                await saveCategoryMapping(payload);
        ...
        <div className="page-container" style={{ padding: "0 20px" }}>
            <div className="page-header" style={{ marginBottom: "25px" }}>
                <h1 style={{ fontSize: "24px", fontWeight: "700", color: "#1e293b" }}>
                    {isEdit ? "Edit Category Mapping Checkbox" : "Add Category Mapping Checkbox"}
                </h1>
            </div>
...
export default AddCategoryMappingCheckbox;
