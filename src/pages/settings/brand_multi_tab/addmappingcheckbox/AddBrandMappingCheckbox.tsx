...
import { getMenus as getBrandMenus } from "../../../../api/multitab/brand_menu.api";
import type { Menu as BrandMultitabMenu } from "../../../../api/multitab/brand_menu.api";
import { getTabHeadings as getBrandTabHeadings } from "../../../../api/multitab/brand_heading.api";
import type { Heading as BrandTabHeading } from "../../../../api/multitab/brand_heading.api";
import { getCheckboxes as getBrandCheckboxes } from "../../../../api/multitab/brand_checkbox.api";
import type { Checkbox as BrandCheckboxMaster } from "../../../../api/multitab/brand_checkbox.api";
import {
    saveMapping as saveBrandMapping,
    updateMapping as updateBrandMapping,
    getMappings as getBrandMappings,
    deleteMapping as deleteBrandMapping
} from "../../../../api/multitab/brand_mapping.api";

const AddBrandMappingCheckbox = () => {
    // Data Lists
    ...
    const loadInitialData = async () => {
        try {
            setLoading(true);
            const [bData, mData, hData, cData, maps] = await Promise.all([
                getBrands(),
                getBrandMenus(),
                getBrandTabHeadings(),
                getBrandCheckboxes(),
            ...
            setBrands(Array.isArray(bData) ? bData : (bData as any)?.data || []);
            setMenus(Array.isArray(mData) ? mData : (mData as any)?.data || []);
            setHeadings(Array.isArray(hData) ? hData : (hData as any)?.data || []);
            setCheckboxes(Array.isArray(cData) ? cData : (cData as any)?.data || []);
            setMappings(Array.isArray(maps) ? maps : (maps as any)?.data || []);
        } catch (err) {
    ...
    const loadMappings = async () => {
        try {
            setLoading(true);
            const data = await getBrandMappings();
            setMappings(Array.isArray(data) ? data : (data as any)?.data || []);
        } catch (error) {
    ...
    const handleEdit = (m: any) => {
        setBrandId(m.brand_id?.toString() || "");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!brandId || !menuId || !headingId || selectedCheckboxIds.length === 0) {
        ...
        try {
            ...
            if (isEdit && editId) {
                await updateBrandMapping(editId, payload);
                alert("Mapping updated successfully ✅");
            } else {
                await saveBrandMapping(payload);
        ...
        <div className="page-container" style={{ padding: "0 20px" }}>
            <div className="page-header" style={{ marginBottom: "25px" }}>
                <h1 style={{ fontSize: "24px", fontWeight: "700", color: "#1e293b" }}>
                    {isEdit ? "Edit Brand Mapping Checkbox" : "Add Brand Mapping Checkbox"}
                </h1>
            </div>
                    ...
                    <div style={{ display: "flex", gap: "25px", marginBottom: "40px" }}>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div className="custom-select-container" style={{ position: "relative" }}>
                                ...
                                >
                                    <span>{sectors.find(s => s.id.toString() === sectorId)?.title || ""}</span>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div className="custom-select-container" style={{ position: "relative" }}>
                                ...
                                >
                                    <span>{brands.find(b => b.id.toString() === brandId)?.brand_name || ""}</span>
                                    <span style={{ fontSize: "12px", color: "#64748b" }}>▼</span>
                                </div>
                                    ...
                                    <div className="custom-dropdown-menu" style={{
                                        ...
                                        <div style={{ padding: "4px" }}>
                                            {brands.filter(b => b.brand_name.toLowerCase().includes(brandSearch.toLowerCase())).map(b => (
                                                <div
                                                    key={b.id}
                                                    onClick={() => { setBrandId(b.id.toString()); setShowBrandDropdown(false); setBrandSearch(""); }}
                                                    style={{ padding: "10px 12px", cursor: "pointer", borderRadius: "4px", fontSize: "14px", background: brandId === b.id.toString() ? "#f3f4f6" : "transparent" }}
                                                    onMouseEnter={(e) => e.currentTarget.style.background = "#f9fafb"}
                                                    onMouseLeave={(e) => e.currentTarget.style.background = brandId === b.id.toString() ? "#f3f4f6" : "transparent"}
                                                >
                                                    {b.brand_name}
                                                </div>
                                            ))}
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div className="custom-select-container" style={{ position: "relative" }}>
                                ...
                                >
                                    {(menuId && menus.find(m => m.id.toString() === menuId)?.menu_name) || ""}
                                    <span style={{ fontSize: "12px", color: "#64748b" }}>▼</span>
                                </div>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div className="custom-select-container" style={{ position: "relative" }}>
                                ...
                                >
                                    {(headingId && headings.find(h => h.id.toString() === headingId)?.heading_name) || ""}
                                    <span style={{ fontSize: "12px", color: "#64748b" }}>▼</span>
                                </div>
                    ...
                    <table className="redesign-table">
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                mappings.map((m, idx) => {
                                    const currentBrand = brands.find(b => b.id === m.brand_id);
                                    ...
                                    return (
                                        <tr key={m.id}>
                                            <td>{idx + 1}</td>
                                            <td style={{ fontWeight: "600", color: "#64748b" }}>{currentBrand?.brand_name}</td>
...
export default AddBrandMappingCheckbox;
