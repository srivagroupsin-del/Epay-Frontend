...
/* =========================
   GET CATEGORY BY ID
========================= */
export const getCategoryById = async (id: number | string): Promise<CategoryRow> => {
  try {
    const json = await http(`/categories/${id}`);
    return json.data ?? json;
  } catch (err) {
    console.warn(`Direct fetch failed for category ${id}, falling back to list filter`, err);
    // Fallback: Fetch all categories and find the one with matching ID
