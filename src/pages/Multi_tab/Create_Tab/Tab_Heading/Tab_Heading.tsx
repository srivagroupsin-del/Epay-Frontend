...
import "./Tab_Heading.css";
import {
    getMenus,
    createTabHeading,
    getTabHeadings as getHeadingsByTab,
    updateTabHeading,
    deleteTabHeading,
    type MultitabMenu as Menu,
    type TabHeading,
...
const TabHeadingMaster: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        formData.append("title_name", form.heading_name); // Title field is hidden, using heading_name as fallback
        formData.append("description", form.description);
        formData.append("status", form.status === 1 ? "active" : "inactive");
        ...
        try {
            ...
            if (isEdit && editId) {
                ...
                alert("Tab Heading updated successfully ✅");
            } else {
                await createTabHeading(formData);
                alert("Tab Heading created successfully ✅");
            }
    ...
    const handleToggleStatus = async (item: TabHeading) => {
        ...
        try {
            ...
            formData.append("multitab_menu_id", String(item.multitab_menu_id || menuId));
            formData.append("heading_name", item.heading_name);
            formData.append("title_name", item.heading_name); // Heading name as fallback for title_name
            formData.append("description", item.description || "");
            formData.append("status", active ? "inactive" : "active");
