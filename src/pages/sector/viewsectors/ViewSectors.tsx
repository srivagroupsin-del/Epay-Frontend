...
import "../../sectortitle/viewsectortitle/sectorTitleList.css";
// Note: sectorTitleList.css contains .page-jump-box, .jump-input, .pagination-container, .pagination-btn
import type { Sector } from "../../../api/sectors.api";
import { getSectors, deleteSector } from "../../../api/sectors.api";
...
const ViewSectors = () => {
  ...
  const sectorTitleId = searchParams.get("sectorTitleId");
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        let filteredData = allSectors;
        if (sectorTitleId) {
          filteredData = allSectors.filter(s => String(s.sector_title_id) === sectorTitleId);
        }
  ...
  const handleDelete = (id: number) => {
    confirmDelete(async () => {
      try {
        showLoader("Deleting sector...");
        await deleteSector(id);
        showDeleteSuccess("Sector has been deleted successfully.", "Deleted Successfully!");
  ...
  const filteredRows = rows
    .filter(item => {
      const name = item.sector_name || item.name || "";
      return name.toLowerCase().startsWith(search.toLowerCase());
    })
    .sort((a, b) => {
      const nameA = a.sector_name || a.name || "";
      const nameB = b.sector_name || b.name || "";
  ...
  return (
    <div className="page-container">
      ...
      <MultitabContentLoader menuTitle="Sector">
        {/* HEADER */}
        <div className="page-header">
          <div>
            <h2>View Sector List</h2>
          </div>
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th style={{ width: '20%', textAlign: 'left' }}>Sector Title</th>
                  <th style={{ width: '25%', textAlign: 'left' }}>Sector Name</th>
                ...
                {paginatedRows.length === 0 ? (
                  ...
                  paginatedRows.map((item, index) => (
                    <tr key={item.id}>
                      ...
                      {/* 2nd: Sector Name */}
                      <td
                        style={{ fontWeight: '600', color: '#323da7', cursor: 'pointer' }}
                        onClick={() => useBusinessStore.getState().setBusiness(item.sector_name || item.name)}
                      >
                      ...
                      <td style={{ textAlign: 'center' }}>
                        {item.image ? (
                          ...
                          <div style={{ width: '45px', height: '45px', background: '#f3f4f6', borderRadius: '8px', margin: '0 auto' }} />
                        )}
                      </td>
                      ...
                      <td style={{ textAlign: 'center' }}>
                        <div className="action-icons" style={{ justifyContent: 'center' }}>
                          <Link
                            to={`/edit-sector/${item.id}?page=${currentPage}`}
                            state={item}
